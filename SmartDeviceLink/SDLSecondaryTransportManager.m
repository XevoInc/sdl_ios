...
typedef NS_ENUM(NSInteger, SDLTransportType) {
    SDLTransportTypeDisabled,   // only for Secondary Transport
    SDLTransportTypeIAP,
    SDLTransportTypeTCP
};
typedef NSNumber SDLTransportTypeBox;
...
@property (weak, nonatomic) SDLAbstractProtocol *primaryProtocol;
...
- (instancetype)initWithProtocol:(SDLAbstractProtocol *)primaryProtocol {
    self = [super init];
    if (!self) {
        return nil;
    }
...
/// Type of secondary transport. If 'SDLTransportTypeDisabled' then secondary transport is disabled.
@property (assign, nonatomic) SDLTransportType transportType;
/// instance of the transport for secondary transport
@property (nullable, strong, nonatomic) SDLAbstractTransport *secondaryTransport;
/// instance of the protocol that runs on secondary transport
@property (nullable, strong, nonatomic) SDLAbstractProtocol *secondaryProtocol;
...
- (instancetype)initWithStreamingProtocolListener:(id<SDLStreamingProtocolListener>)streamingProtocolListener {
    ...
    _transportType = SDLTransportTypeDisabled;
...
- (void)startWithProtocol:(SDLAbstractProtocol *)primaryProtocol {
...
- (void)didEnterStateConfigured {
    if ((self.transportType == SDLTransportTypeTCP && [self sdl_isTCPReady]) ||
        self.transportType == SDLTransportTypeIAP) {
...
- (void)sdl_stopManager {
    ...
    self.assignedTransport = [@{@(SDLServiceTypeAudio):@(SDLTransportClassInvalid),
                                @(SDLServiceTypeVideo):@(SDLTransportClassInvalid)} mutableCopy];
    self.transportType = SDLTransportTypeDisabled;
...
- (void)sdl_configureManager:(nullable NSArray<SDLTransportTypeBox *> *)secondaryTransports
    ...
    SDLTransportType secondaryTransportType;
    if (secondaryTransports == nil || [secondaryTransports count] == 0) {
        ...
        secondaryTransportType = SDLTransportTypeDisabled;
    } else {
        // current proposal says the list should contain only one element
        SDLTransportTypeBox *transportType = [secondaryTransports objectAtIndex:0];
        secondaryTransportType = [transportType integerValue];
    }

    SDLTransportType primaryTransportType = [self sdl_getTransportTypeFromProtocol:self.primaryProtocol];
    ...
    } else if (secondaryTransportType == SDLTransportTypeIAP) {
        SDLLogW(@"Starting IAP as secondary transport, which does not usually happen");
    }

    self.transportType = secondaryTransportType;
...
- (void)sdl_handleTransportEventUpdate {
    ...
    if (self.transportType != SDLTransportTypeTCP) {
        return;
    }
...
- (void)sdl_handleAppBecomeActive {
    if (([self.stateMachine isCurrentState:SDLSecondaryTransportStateConfigured])
        && self.transportType == SDLTransportTypeTCP && [self sdl_isTCPReady]) {
...
- (void)sdl_handleAppResignedActive {
    if ([self sdl_isTransportOpened] && self.transportType == SDLTransportTypeTCP) {
...
- (void)sdl_handleTransportUpdate:(BOOL)primaryAvailable secondaryAvailable:(BOOL)secondaryAvailable {
    ...
    [self sdl_updateService:SDLServiceTypeAudio
      ...
      transportUpdatedBlock:^(SDLAbstractProtocol * _Nullable oldProtocol, SDLAbstractProtocol * _Nullable newProtocol) {
...
- (void)sdl_updateService:(UInt8)service
    ...
    transportUpdatedBlock:(void (^)(SDLAbstractProtocol * _Nullable oldProtocol, SDLAbstractProtocol * _Nullable newProtocol))transportUpdatedBlock {
...
- (nullable SDLAbstractProtocol *)sdl_getProtocolFromTransportClass:(SDLTransportClass)transportClass {
...
- (void)sdl_onStartServiceACKReceived:(NSNotification *)notification {
    ...
    NSMutableArray<SDLTransportTypeBox *> *secondaryTransports = nil;
    if (payload.secondaryTransports != nil) {
        ...
        for (NSString *transportString in payload.secondaryTransports) {
            SDLTransportType transport = [self sdl_convertTransportType:transportString];
...
- (SDLTransportType)sdl_convertTransportType:(NSString *)transportString {
    if ([transportString isEqualToString:@"TCP_WIFI"]) {
        return SDLTransportTypeTCP;
        ...
        return SDLTransportTypeIAP;
    } else {
        // "SPP_BLUETOOTH" and "AOA_USB" are not used
        return SDLTransportTypeDisabled;
    }
}
...
- (SDLTransportType)sdl_getTransportTypeFromProtocol:(SDLAbstractProtocol *)protocol {
...
- (BOOL)sdl_connectSecondaryTransport {
    ...
    switch (self.transportType) {
        case SDLTransportTypeTCP:
            return [self sdl_startTCPSecondaryTransport];
        case SDLTransportTypeIAP:
            return [self sdl_startIAPSecondaryTransport];
        default:
            SDLLogW(@"Unknown transport type for secondary transport: %ld", (long)self.transportType);
            return NO;
    }
}
