...
@interface SDLTCPLegacyTransport () {
...
@implementation SDLTCPLegacyTransport
...
- (void)connect {
    ...
    [[NSOperationQueue mainQueue] addOperationWithBlock:^{
        ...
        int sock_fd = call_socket([self.parent.hostName UTF8String], [self.parent.portNumber UTF8String]);
        if (sock_fd < 0) {
...
static void TCPCallback(CFSocketRef socket, CFSocketCallBackType type, CFDataRef address, const void *data, void *info) {
    if (kCFSocketConnectCallBack == type) {
        SDLTCPLegacyTransport *transport = (__bridge SDLTCPLegacyTransport *)info;
        [transport.parent.delegate onTransportConnected];
        ...
        if (CFDataGetLength((CFDataRef)data) <= 0) {
            ...
            [transport.parent.delegate onTransportDisconnected];
        ...
        [transport.parent.delegate onDataReceived:convertedData];
    } else {
